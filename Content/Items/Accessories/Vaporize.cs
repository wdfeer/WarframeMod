...
public class Vaporize : ModItem
{
    ...
    {
        Tooltip.SetDefault($"All minions will deal {DAMAGE} damage to a nearby enemy every {COOLDOWN / 60} seconds");
    }
    ...
    {
        return Main.npc.Where(npc => !npc.friendly && npc.CanBeChasedBy() && npc.Center.Distance(player.Center) < MAX_ATTACK_DISTANCE).ToArray();
    }
    void Fire(Player player, IList<NPC> targets)
    {
        IEnumerable<Projectile> minions = Main.projectile.Where(proj => proj.friendly && proj.owner == player.whoAmI && proj.minionSlots > 0);
